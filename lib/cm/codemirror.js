...
(function (global, factory) {
  ...
  function getMode(options, spec) {
    ...
    if (spec.modeProps) { for (const prop$1 in spec.modeProps)
  ...
  function buildLineContent(cm, lineView) {
    ...
    const builder = {pre: eltP("pre", [content], "CodeMirror-line"), content: content,
                   col: 0, pos: 0, cm: cm,
                   trailingSpace: false,
  ...
  function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
    ...
    if (style || startStyle || endStyle || mustWrap || css || attributes) {
      ...
      if (attributes) {
        for (const attr in attributes) { if (attributes.hasOwnProperty(attr) && attr != "style" && attr != "class")
  ...
  function insertLineWidgetsFor(cm, line, lineView, dims, allowAbove) {
    ...
    for (let i = 0, ws = line.widgets; i < ws.length; ++i) {
      const widget = ws[i], node = elt("div", [widget.node], "CodeMirror-linewidget" + (widget.className ? " " + widget.className : ""));
  ...
  function intoCoordSystem(cm, lineObj, rect, context, includeWidgets) {
    ...
    if (context == "page" || context == "window") {
      ...
      const xOff = lOff.left + (context == "window" ? 0 : pageScrollX(doc(cm)));
  ...
  function drawSelectionRange(cm, range, output) {
    // MARK 动态计算 margin
    let marginRight = 0;
    if (displayWidth(cm) >= 860) {
       marginRight = ( displayWidth(cm) - 800 );  // 800px 是 max-width
    } else {
       marginRight = 60; // 30px 是 margin-right
    }
    const display = cm.display, doc = cm.doc;
    const fragment = document.createDocumentFragment();
    const padding = paddingH(cm.display), leftSide = padding.left;
    const rightSide = Math.max(display.sizerWidth, displayWidth(cm) - display.sizer.offsetLeft) - marginRight - padding.right; //MARK 
    // const rightSide = Math.max(display.sizerWidth, displayWidth(cm) - display.sizer.offsetLeft) - padding.right;  //这是原版 我剪去了56px 是margin
  ...
  // MARK Cursor-blinking visibility修改opacity
  function restartBlink(cm) {
    ...
    if (cm.options.cursorBlinkRate > 0)
      { display.blinker = setInterval(function () {
        ...
        display.cursorDiv.style.opacity = (on = !on) ? "1" : "0";
  ...
  function maybeScrollWindow(cm, rect) {
    ...
    const doc = display.wrapper.ownerDocument;
  ...
  function endOperation_W2(op) {
    ...
    const takeFocus = op.focus && op.focus == activeElt(root(cm));
  ...
  function selectionSnapshot(cm) {
    ...
    const active = activeElt(root(cm));
    if (!active || !contains(cm.display.lineDiv, active)) { return null }
    const result = {activeElt: active};
    if (window.getSelection) {
      const sel = win(cm).getSelection();
  ...
  function restoreSelection(snapshot) {
    ...
    if (!/^(INPUT|TEXTAREA)$/.test(snapshot.activeElt.nodeName) &&
      ...
      const doc = snapshot.activeElt.ownerDocument;
  ...
  function skipAtomicInSelection(doc, sel, bias, mayClear) {
    ...
    for (let i = 0; i < sel.ranges.length; i++) {
      ...
      const newHead = range.head == range.anchor ? newAnchor : skipAtomic(doc, range.head, old && old.head, bias, mayClear);
  ...
  function normalizeKeyMap(keymap) {
    const copy = {};
    for (const keyname in keymap) { if (keymap.hasOwnProperty(keyname)) {
  ...
  function moveVisually(cm, line, start, dir) {
    ...
    const searchInVisualLine = function (partPos, dir, wrappedLineExtent) {
      const getRes = function (ch, moveInStorageOrder) { return moveInStorageOrder
  ...
  function applyTextInput(cm, inserted, deleted, sel, origin) {
    ...
    for (let i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
      ...
      const changeEvent = {from: from, to: to, text: multiPaste ? multiPaste[i$1 % multiPaste.length] : textLines,
  ...
  function findPosV(cm, pos, dir, unit) {
    ...
    if (unit == "page") {
      const pageSize = Math.min(cm.display.wrapper.clientHeight, win(cm).innerHeight || doc(cm).documentElement.clientHeight);
  ...
  ContentEditableInput.prototype.init = function (display) {
    ...
    function onCopyCut(e) {
      ...
      const hadFocus = activeElt(rootNode(div));
      selectInput(te);
      setTimeout(function () {
  ...
  function fromTextArea(textarea, options) {
    ...
    if (options.autofocus == null) {
      const hasFocus = activeElt(rootNode(textarea));
