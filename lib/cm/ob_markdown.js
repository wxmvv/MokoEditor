...
!(function () {
	var N = {
		...
		M = { autoSelfClosers: {}, implicitlyClosed: {}, contextGrabbers: {}, doNotIndent: {}, allowUnquoted: !1, allowMissing: !1, allowMissingTagName: !1, caseFold: !1 };
	CodeMirror.defineMode("xml", function (t, e) {
		...
		function d(e, r) {
			...
			if ("<" == n)
				return e.eat("!")
								...
								(function n(o) {
									return function (t, e) {
										for (var r; null != (r = t.next()); ) {
											if ("<" == r) return (e.tokenize = n(o + 1)), e.tokenize(t, e);
											if (">" == r) {
												if (1 != o) return (e.tokenize = n(o - 1)), e.tokenize(t, e);
												e.tokenize = d;
												break;
											}
										}
					...
					: ((a = e.eat("/") ? "closeTag" : "openTag"), (r.tokenize = s), "tag bracket");
			if ("&" != n) return e.eatWhile(/[^&<]/), null;
			n = e.eat("#") ? (e.eat("x") ? e.eatWhile(/[a-fA-F\d]/) && e.eat(";") : e.eatWhile(/[\d]/) && e.eat(";")) : e.eatWhile(/[\w\.\-:]/) && e.eat(";");
			return n ? "atom" : "error";
		}
		function s(t, e) {
			var r = t.next();
			if (">" == r || ("/" == r && t.eat(">"))) return (e.tokenize = d), (a = ">" == r ? "endTag" : "selfcloseTag"), "tag bracket";
			if ("=" == r) return (a = "equals"), null;
			if ("<" != r)
				return /[\'\"]/.test(r)
					? ((e.tokenize = ((n = r), (o.isInAttribute = !0), o)), (e.stringStartCol = t.column()), e.tokenize(t, e))
					: (t.match(/^[^\s\u00a0=<>\"\']*[^\s\u00a0=<>\"\'\/]/), "word");
		...
		function g(t, e) {
			for (var r; ; ) {
				if (!t.context) return;
				if (((r = t.context.tagName), !u.contextGrabbers.hasOwnProperty(c(r)) || !u.contextGrabbers[c(r)].hasOwnProperty(c(e)))) return;
				m(t);
			}
		}
		function p(t, e, r) {
			return "openTag" == t ? ((r.tagStart = e.column()), h) : "closeTag" == t ? x : p;
		}
		function h(t, e, r) {
			return "word" == t ? ((r.tagName = e.current()), (i = "tag"), w) : u.allowMissingTagName && "endTag" == t ? ((i = "tag bracket"), w(t, 0, r)) : ((i = "error"), h);
		}
		function x(t, e, r) {
			if ("word" != t) return u.allowMissingTagName && "endTag" == t ? ((i = "tag bracket"), b(t, 0, r)) : ((i = "error"), k);
			e = e.current();
			return (
				r.context && r.context.tagName != e && u.implicitlyClosed.hasOwnProperty(c(r.context.tagName)) && m(r),
				(r.context && r.context.tagName == e) || !1 === u.matchClosing ? ((i = "tag"), b) : ((i = "tag error"), k)
			);
		}
		function b(t, e, r) {
		...
		function w(t, e, r) {
			...
			var n = r.tagName,
				o = r.tagStart;
			return (r.tagName = r.tagStart = null), "selfcloseTag" == t || u.autoSelfClosers.hasOwnProperty(c(n)) ? g(r, n) : (g(r, n), (r.context = new f(r, n, o == r.indented))), p;
		}
		function C(t, e, r) {
			return "equals" == t ? T : (u.allowMissing || (i = "error"), w(t, 0, r));
		}
		function T(t, e, r) {
			return "string" == t ? v : "word" == t && u.allowUnquoted ? ((i = "string"), w) : ((i = "error"), w(t, 0, r));
		}
		function v(t, e, r) {
		...
		return (
			(d.isInText = !0),
			{
				startState: function (t) {
					var e = { tokenize: d, state: p, indented: t || 0, tagName: null, tagStart: null, context: null };
				...
				token: function (t, e) {
					...
					return (r || a) && "comment" != r && ((i = null), (e.state = e.state(a || r, t, e)), i && (r = "error" == i ? r + " error" : i)), r;
				},
				indent: function (t, e, r) {
					...
					if (t.tokenize != s && t.tokenize != d) return r ? r.match(/^(\s*)/)[0].length : 0;
	...
	CodeMirror.defineMode(
		...
		function (i, t) {
			var c = CodeMirror.getMode(i, {
				...
				e = t && t.tags,
				a = t && t.scriptTypes;
			if ((o(l, s), e && o(e, s), a)) for (var n = a.length - 1; 0 <= n; n--) s.script.unshift(["type", a[n].matches, a[n].mode]);
			function u(t, e) {
				var a,
					o,
					r,
					...
					l = /\btag\b/.test(n);
				return (
					l && !/[<>\s\/]/.test(t.current()) && (a = e.htmlState.tagName && e.htmlState.tagName.toLowerCase()) && s.hasOwnProperty(a)
						? (e.inTag = a + " ")
						  ...
						  (e.token = function (t, e) {
								return t.match(o, !1)
									...
									: ((a = t),
									  ...
									  (t = a.current()),
									  -1 < (e = t.search(n)) ? a.backUp(t.length - e) : t.match(/<\/?$/) && (a.backUp(t.length), a.match(n, !1) || a.match(t)),
									  l);
						  }),
						  (e.localMode = l),
						  (e.localState = CodeMirror.startState(l, c.indent(e.htmlState, "", ""))))
			...
			return {
				startState: function () {
					return { token: u, inTag: null, localMode: null, localState: null, htmlState: CodeMirror.startState(c) };
				},
				copyState: function (t) {
					var e;
					return (
						t.localState && (e = CodeMirror.copyState(t.localMode, t.localState)),
						{ token: t.token, inTag: t.inTag, localMode: t.localMode, localState: e, htmlState: CodeMirror.copyState(c, t.htmlState) }
					);
				},
				token: function (t, e) {
					return e.token(t, e);
				...
				indent: function (t, e, a) {
					return !t.localMode || /^\s*<\//.test(e) ? c.indent(t.htmlState, e, a) : t.localMode.indent ? t.localMode.indent(t.localState, e, a) : CodeMirror.Pass;
				},
		...
		function (cmCfg, modeCfg) {
			...
			function htmlBlock(stream, state) {
				...
				if (!htmlModeMissing) {
					var inner = CodeMirror.innerMode(htmlMode, state.htmlState);
					if (
						(inner.mode.name == "xml" && inner.state.tagStart === null && !inner.state.context && inner.state.tokenize.isInText) ||
